...
class ADocument extends UnmodifiableListView<Map> implements Clonable {
  ...
  factory ADocument.fromText(String text, {String author}) {
    ...
    if (text != null && text.isNotEmpty) {
      var builder = Changeset.create(doc, author: author)..insert(text);
  ...
  ADocument compact() {
    var newPool = [];
    var lines = map((l) => new ComponentList.unpack(new AString.unpack(l), pool).pack(newPool)).toList(growable: false);
