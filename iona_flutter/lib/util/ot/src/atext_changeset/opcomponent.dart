...
class OpComponent {
  ...
  OpComponent.keep(this.chars, this.lines)
      : opcode = KEEP,
        charBank = '',
        attribs = new AttributeList() {
    _validate();
  }
  OpComponent.format(this.chars, this.lines, this.attribs)
      : opcode = KEEP,
        charBank = '' {
    _validate();
  }
  ...
  void _validate() {
    ...
    if (isInsert || isRemove) {
      if (lines > 0 && !charBank.endsWith('\n')) {
        throw new Exception('for multiline components charbank $charBank should end up with newline');
      }
      if (chars != charBank.length) {
        throw new Exception(
            'charBank length should match chars in operation: expected $chars got [${charBank.length}]');
      }
    }
  }
  ...
  OpComponent sliceLeft(int N, int L) {
    if (chars < N || lines < L) {
      throw new Exception('op $charBank is too short for sliceLeft: $chars < $N or $lines < $L');
    }
  ...
  OpComponent append(OpComponent otherCmp) {
    if (otherCmp.isNotEmpty) {
      ...
      return new OpComponent(
          opcode, chars + otherCmp.chars, lines + otherCmp.lines, attribs, charBank + otherCmp.charBank);
    } else {
      return this;
    }
  }
  ...
  bool equalsButOpcode(other) =>
      other is OpComponent &&
      chars == other.chars &&
      lines == other.lines &&
  ...
  AString pack(List pool) {
    ...
    return new AString(
        atts:
            attribs.pack(pool) + (lines > 0 ? ('|' + _util.toString36(lines)) : '') + opcode + _util.toString36(chars),
        text: charBank,
        dLen: deltaLen);
  }
