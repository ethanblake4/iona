...
import 'package:iona_flutter/util/io/progress.dart';
...
Which which(String appname,
        {bool first = true, bool verbose = false, bool extensionSearch = true, Progress progress}) =>
    _Which().which(appname, first: first, extensionSearch: extensionSearch, progress: progress);
...
class _Which {
  ...
  Which which(String appname, {bool extensionSearch, bool first = true, Progress progress}) {
    final results = Which();
    try {
      ...
      for (final path in PATH) {
        final fullpath = _appExists(path, appname, extensionSearch: extensionSearch);
        if (fullpath != null) {
  ...
  /// no extension exits.
  String _appExists(String pathTo, String appname, {bool extensionSearch}) {
    final pathToAppname = join(pathTo, appname);
    if (exists(pathToAppname)) {
      return pathToAppname;
    }
    if (LocalPlatform().isWindows && extensionSearch && extension(appname).isEmpty) {
