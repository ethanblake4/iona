...
class Env {
  ...
  /// we inherited from the parent process.
  static void reset() {
    _self = Env._internal();
    env = _self;
  }
  ...
  String get HOME {
    ...
    if (home == null) {
      if (Platform.isWindows) {
        throw Exception("Unable to find the 'APPDATA' enviroment variable. "
            'Please ensure it is set and try again.');
      } else {
        throw Exception("Unable to find the 'HOME' enviroment variable. "
            'Please ensure it is set and try again.');
      }
    }
    return home;
