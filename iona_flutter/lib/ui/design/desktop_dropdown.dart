...
class DesktopDropdownMenuItem<T> extends _DropdownMenuItemContainer {
...
class _DropdownMenuItemButtonState<T> extends State<_DropdownMenuItemButton<T>> {
  ...
  void _handleOnTap() {
    final DesktopDropdownMenuItem<T> dropdownMenuItem = widget.route.items[widget.itemIndex].item;
...
class DMenuItem<T> extends SingleChildRenderObjectWidget {
  ...
  final DesktopDropdownMenuItem<T> item;
...
/// An inherited widget that causes any descendant [DesktopDropdownButton]
/// widgets to not include their regular underline.
///
/// This is used by [DataTable] to remove the underline from any
/// [DesktopDropdownButton] widgets placed within material data tables, as
...
class DropdownButtonHideUnderline extends InheritedWidget {
  ...
  /// Returns whether the underline of [DesktopDropdownButton] widgets should
  /// be hidden.
...
class DesktopDropdownButton<T> extends StatefulWidget {
  ...
  DesktopDropdownButton({
    ...
    // DropdownButtonFormField.
          ...
          items == null ||
              items.isEmpty ||
              value == null ||
              items.where((DesktopDropdownMenuItem<T> item) {
  ...
  @override
  _DesktopDropdownButtonState<T> createState() => _DesktopDropdownButtonState<T>();
}
...
class _DesktopDropdownButtonState<T> extends State<DesktopDropdownButton<T>> with WidgetsBindingObserver {
  ...
  @override
  void didUpdateWidget(DesktopDropdownButton<T> oldWidget) {
  ...
  void _updateSelectedIndex() {
    ...
    assert(widget.value == null ||
        widget.items.where((DesktopDropdownMenuItem<T> item) => item.value == widget.value).length == 1);
    _selectedIndex = null;
...
/// A convenience widget that makes a [DesktopDropdownButton] into a [FormField].
class DropdownButtonFormField<T> extends FormField<T> {
  /// Creates a [DesktopDropdownButton] widget that is a [FormField], wrapped in an
  /// [InputDecorator].
  ///
  ...
  DropdownButtonFormField({
    Key key,
    @required List<DesktopDropdownMenuItem<T>> items,
    ...
    bool autovalidate = false,
        ...
        assert(isDense != null),
        assert(isExpanded != null),
        assert(itemHeight == null || itemHeight >= 24.0),
        assert(autofocus != null),
