...
import 'package:analyzer_plugin/protocol/protocol_common.dart' show CompletionSuggestion, CompletionSuggestionKind;
import 'package:analyzer_plugin/utilities/change_builder/change_builder_dart.dart';
import 'package:analyzer_plugin/utilities/completion/relevance.dart';
import 'package:analyzer_plugin/utilities/range_factory.dart';
import 'package:iona_flutter/plugin/dart/completion/dart/completion_dart.dart';
import 'package:iona_flutter/plugin/dart/completion/dart/utilities.dart';
import 'package:iona_flutter/plugin/dart/completion/protocol/protocol_dart.dart';
import 'package:iona_flutter/plugin/dart/completion/protocol/protocol_dart.dart' as protocol;
import 'package:iona_flutter/plugin/dart/completion/protocol/protocol_server.dart' show getReturnTypeString;
import 'package:iona_flutter/plugin/dart/completion/utilities/comment.dart';
import 'package:iona_flutter/plugin/dart/completion/utilities/flutter.dart';
import 'package:iona_flutter/plugin/dart/computer/computer_hover.dart';
import 'package:meta/meta.dart';

import '../../utils/extensions/ast.dart';
import '../../utils/extensions/element.dart';
import '../service/feature_computer.dart';
...
class MemberSuggestionBuilder {
  ...
  void addSuggestionForAccessor({@required PropertyAccessorElement accessor, @required double inheritanceDistance}) {
    if (accessor.isAccessibleIn(request.libraryElement)) {
      ...
      if (_shouldAddSuggestion(member)) {
        builder.suggestAccessor(accessor, inheritanceDistance: inheritanceDistance);
      }
    }
  }
  ...
  void addSuggestionForMethod(
      {@required MethodElement method, CompletionSuggestionKind kind, @required double inheritanceDistance}) {
    if (method.isAccessibleIn(request.libraryElement) && _shouldAddSuggestion(method)) {
      builder.suggestMethod(method, kind: kind, inheritanceDistance: inheritanceDistance);
    }
  }
  ...
  bool _shouldAddSuggestion(Element element) {
    ...
    var alreadyGenerated = _completionTypesGenerated.putIfAbsent(identifier, () => _COMPLETION_TYPE_NONE);
    if (element is MethodElement) {
      // Anything shadows a method.
      if (alreadyGenerated != _COMPLETION_TYPE_NONE) {
        return false;
      }
      _completionTypesGenerated[identifier] = _COMPLETION_TYPE_FIELD_OR_METHOD_OR_GETSET;
...
class SuggestionBuilder {
  ...
  final Map<String, CompletionSuggestion> _suggestionMap = <String, CompletionSuggestion>{};
  ...
  String get _containingMemberName {
    if (!_hasContainingMemberName) {
      ...
      if (request.dotTarget is SuperExpression) {
        var containingMethod = request.target.containingNode.thisOrAncestorOfType<MethodDeclaration>();
  ...
  /// static accessor.
  void suggestAccessor(PropertyAccessorElement accessor, {@required double inheritanceDistance}) {
    assert(accessor.enclosingElement is ClassElement || accessor.enclosingElement is ExtensionElement);
    if (accessor.isSynthetic) {
      ...
      if (request.useNewRelevance) {
        ...
        var contextType = featureComputer.contextTypeFeature(request.contextType, type);
        var elementKind = _computeElementKind(accessor);
        var hasDeprecated = featureComputer.hasDeprecatedFeature(accessor);
        var isConstant = request.inConstantContext ? featureComputer.isConstantFeature(accessor) : -1.0;
        var startsWithDollar = featureComputer.startsWithDollarFeature(accessor.name);
        var superMatches = featureComputer.superMatchesFeature(_containingMemberName, accessor.name);
        ...
        relevance = accessor.hasOrInheritsDeprecated ? DART_RELEVANCE_LOW : _computeOldMemberRelevance(accessor);
  ...
  void suggestCatchParameter(LocalVariableElement parameter) {
    ...
    int relevance;
    if (request.useNewRelevance) {
      var contextType = request.featureComputer.contextTypeFeature(request.contextType, variableType);
      var elementKind = _computeElementKind(parameter);
      var isConstant = request.inConstantContext ? request.featureComputer.isConstantFeature(parameter) : -1.0;
      relevance = toRelevance(weightedAverage([contextType, elementKind, isConstant], [1.0, 1.0, 1.0]), 800);
      listener?.computedFeatures(contextType: contextType);
    } else {
      relevance = _computeOldMemberRelevance(parameter);
      relevance = request.opType.returnValueSuggestionsFilter(variableType, relevance);
      if (relevance == null) {
        return;
      }
    }

    _add(_createSuggestion(parameter, elementKind: protocol.ElementKind.PARAMETER, relevance: relevance));
  }
  ...
  void suggestClass(ClassElement classElement,
      {CompletionSuggestionKind kind = CompletionSuggestionKind.INVOCATION, String prefix}) {
    int relevance;
    if (request.useNewRelevance) {
      relevance = _computeTopLevelRelevance(classElement, elementType: _instantiateClassElement(classElement));
    } else if (classElement.hasOrInheritsDeprecated) {
      relevance = DART_RELEVANCE_LOW;
    } else {
      relevance =
          request.opType.typeNameSuggestionsFilter(_instantiateClassElement(classElement), DART_RELEVANCE_DEFAULT);
      if (relevance == null) {
        return;
      }
    }

    _add(_createSuggestion(classElement, kind: kind, prefix: prefix, relevance: relevance));
  }
  ...
  void suggestConstructor(ConstructorElement constructor,
      {CompletionSuggestionKind kind = CompletionSuggestionKind.INVOCATION, bool hasClassName = false, String prefix}) {
    ...
    if (request.useNewRelevance) {
      relevance = _computeTopLevelRelevance(constructor, elementType: returnType);
    } else {
      relevance = constructor.hasOrInheritsDeprecated ? DART_RELEVANCE_LOW : DART_RELEVANCE_DEFAULT;
      relevance = request.opType.returnValueSuggestionsFilter(returnType, relevance);
      if (relevance == null) {
        return;
      }
    }

    _add(_createSuggestion(constructor, completion: completion, kind: kind, prefix: prefix, relevance: relevance));
  }

  /// Add a suggestion for a top-level [element]. If a [kind] is provided it
  /// will be used as the kind for the suggestion.
  void suggestElement(Element element, {CompletionSuggestionKind kind = CompletionSuggestionKind.INVOCATION}) {
    ...
    } else if (element is FunctionElement && element.enclosingElement is CompilationUnitElement) {
      suggestTopLevelFunction(element, kind: kind);
    } else if (element is FunctionTypeAliasElement) {
      suggestFunctionTypeAlias(element, kind: kind);
    } else if (element is PropertyAccessorElement && element.enclosingElement is CompilationUnitElement) {
  ...
  void suggestEnumConstant(FieldElement constant, {String prefix}) {
    ...
    int relevance;
    if (request.useNewRelevance) {
      relevance = _computeTopLevelRelevance(constant, elementType: constant.type);
    } else if (constant.hasOrInheritsDeprecated) {
      relevance = DART_RELEVANCE_LOW;
    } else {
      relevance =
          request.opType.returnValueSuggestionsFilter(_instantiateClassElement(enumElement), DART_RELEVANCE_DEFAULT);
      if (relevance == null) {
        return;
      }
    }

    _add(_createSuggestion(constant, completion: completion, prefix: prefix, relevance: relevance));
  }
  ...
  void suggestExtension(ExtensionElement extension,
    ...
    int relevance;
    if (request.useNewRelevance) {
      relevance = _computeTopLevelRelevance(extension, elementType: extension.extendedType);
    } else {
      relevance = extension.hasOrInheritsDeprecated ? DART_RELEVANCE_LOW : DART_RELEVANCE_DEFAULT;
    }

    _add(_createSuggestion(extension, kind: kind, prefix: prefix, relevance: relevance));
  }
  ...
  void suggestField(FieldElement field, {@required double inheritanceDistance}) {
    ...
    if (request.useNewRelevance) {
      var featureComputer = request.featureComputer;
      var contextType = featureComputer.contextTypeFeature(request.contextType, field.type);
      var elementKind = _computeElementKind(field);
      var hasDeprecated = featureComputer.hasDeprecatedFeature(field);
      var isConstant = request.inConstantContext ? featureComputer.isConstantFeature(field) : -1.0;
      var startsWithDollar = featureComputer.startsWithDollarFeature(field.name);
      var superMatches = featureComputer.superMatchesFeature(_containingMemberName, field.name);
      relevance = _computeMemberRelevance(
          contextType: contextType,
          elementKind: elementKind,
          hasDeprecated: hasDeprecated,
      ...
      if (request.opType.includeReturnValueSuggestions) {
        relevance = request.opType.returnValueSuggestionsFilter(field.type, relevance);
      }
      if (relevance == null) {
        return;
      }
  ...
  void suggestFieldFormalParameter(FieldElement field) {
    ...
    var relevance = request.useNewRelevance ? Relevance.fieldFormalParameter : DART_RELEVANCE_LOCAL_FIELD;
  ...
  void suggestFunctionCall() {
    ...
    final element = protocol.Element(protocol.ElementKind.METHOD, callString, protocol.Element.makeFlags(),
        location: null, typeParameters: null, parameters: '()', returnType: 'void');
  ...
  void suggestFunctionTypeAlias(FunctionTypeAliasElement functionTypeAlias,
    ...
    if (request.useNewRelevance) {
      relevance = _computeTopLevelRelevance(functionTypeAlias,
          defaultRelevance: 750, elementType: _instantiateFunctionTypeAlias(functionTypeAlias));
    } else if (functionTypeAlias.hasOrInheritsDeprecated) {
      relevance = DART_RELEVANCE_LOW;
    } else {
      relevance =
          functionTypeAlias.library == request.libraryElement ? DART_RELEVANCE_LOCAL_FUNCTION : DART_RELEVANCE_DEFAULT;
    }
    _add(_createSuggestion(functionTypeAlias, kind: kind, prefix: prefix, relevance: relevance));
  }
  ...
  void suggestKeyword(String keyword, {int offset, @required int relevance}) {
    if (request.useNewRelevance) {
      ...
      relevance = toRelevance(request.featureComputer.keywordFeature(keyword, request.opType.completionLocation), 800);
    }
    _add(CompletionSuggestion(
        CompletionSuggestionKind.KEYWORD, relevance, keyword, offset ?? keyword.length, 0, false, false));
  }
  ...
  void suggestLabel(Label label) {
    ...
    if (completion != null && completion.isNotEmpty && completion != '_') {
      var suggestion = CompletionSuggestion(
          ...
          false);
      suggestion.element =
          createLocalElement(request.source, protocol.ElementKind.LABEL, label.label, returnType: NO_RETURN_TYPE);
      _add(suggestion);
    }
  }
  ...
  void suggestLoadLibraryFunction(FunctionElement function) {
    ...
    if (request.useNewRelevance) {
      ...
      relevance = function.hasOrInheritsDeprecated ? DART_RELEVANCE_LOW : DART_RELEVANCE_DEFAULT;
    }
  ...
  void suggestLocalVariable(LocalVariableElement variable) {
    ...
    if (request.useNewRelevance) {
      ...
      var isConstant = request.inConstantContext ? request.featureComputer.isConstantFeature(variable) : -1.0;
      relevance = toRelevance(weightedAverage([contextType, elementKind, isConstant], [1.0, 1.0, 1.0]), 800);
      listener?.computedFeatures(contextType: contextType);
    } else {
  ...
  void suggestMethod(MethodElement method, {CompletionSuggestionKind kind, @required double inheritanceDistance}) {
    ...
    if (request.useNewRelevance) {
      var featureComputer = request.featureComputer;
      var contextType = featureComputer.contextTypeFeature(request.contextType, method.returnType);
      var elementKind = _computeElementKind(method);
      var hasDeprecated = featureComputer.hasDeprecatedFeature(method);
      var isConstant = request.inConstantContext ? featureComputer.isConstantFeature(method) : -1.0;
      var startsWithDollar = featureComputer.startsWithDollarFeature(method.name);
      var superMatches = featureComputer.superMatchesFeature(_containingMemberName, method.name);
      relevance = _computeMemberRelevance(
          contextType: contextType,
          elementKind: elementKind,
          hasDeprecated: hasDeprecated,
      ...
      if (request.opType.includeReturnValueSuggestions) {
        relevance = request.opType.returnValueSuggestionsFilter(method.returnType, relevance);
      }
      if (relevance == null) {
        return;
      }
    ...
    var suggestion = _createSuggestion(method, kind: kind, relevance: relevance);
    if (suggestion != null) {
      if (method.name == 'setState' && flutter.isExactState(method.enclosingElement)) {
  ...
  void suggestName(String name) {
    ...
    _add(CompletionSuggestion(CompletionSuggestionKind.IDENTIFIER,
        request.useNewRelevance ? 500 : DART_RELEVANCE_DEFAULT, name, name.length, 0, false, false));
  }
  ...
  /// completion text.
  void suggestNamedArgument(ParameterElement parameter, {@required bool appendColon, @required bool appendComma}) {
    ...
    if (parameter.isRequiredNamed || parameter.hasRequired) {
      relevance = request.useNewRelevance ? Relevance.requiredNamedArgument : DART_RELEVANCE_NAMED_PARAMETER_REQUIRED;
    } else {
      relevance = request.useNewRelevance ? Relevance.namedArgument : DART_RELEVANCE_NAMED_PARAMETER;
    }

    var suggestion = CompletionSuggestion(
        CompletionSuggestionKind.NAMED_ARGUMENT, relevance, completion, selectionOffset, 0, false, false,
  ...
  Future<void> suggestOverride(SimpleIdentifier targetId, ExecutableElement element, bool invokeSuper) async {
    ...
    if (request.target.containingNode.hasOverride && completion.startsWith(overrideAnnotation)) {
    ...
    var displayText = displayTextBuffer.isNotEmpty ? displayTextBuffer.toString() : null;
    var suggestion = CompletionSuggestion(
  ...
  void suggestPrefix(LibraryElement library, String prefix) {
    ...
    if (request.useNewRelevance) {
      ...
      relevance = library.hasDeprecated ? DART_RELEVANCE_LOW : DART_RELEVANCE_DEFAULT;
    }
    _add(_createSuggestion(library,
        completion: prefix, kind: CompletionSuggestionKind.IDENTIFIER, relevance: relevance));
  }
  ...
  void suggestTopLevelFunction(FunctionElement function,
    ...
    int relevance;
    if (request.useNewRelevance) {
      relevance = _computeTopLevelRelevance(function, elementType: function.returnType);
    } else if (function.hasOrInheritsDeprecated) {
      relevance = DART_RELEVANCE_LOW;
    } else {
      relevance = function.library == request.libraryElement ? DART_RELEVANCE_LOCAL_FUNCTION : DART_RELEVANCE_DEFAULT;
      relevance = request.opType.returnValueSuggestionsFilter(function.type, relevance);
      if (relevance == null) {
        return;
      }
    }

    _add(_createSuggestion(function, kind: kind, prefix: prefix, relevance: relevance));
  }
  ...
  void suggestTopLevelVariable(TopLevelVariableElement variable,
    ...
    int relevance;
    if (request.useNewRelevance) {
      relevance = _computeTopLevelRelevance(variable, elementType: variable.type);
    } else {
      var type = variable.type;
      relevance = variable.hasOrInheritsDeprecated ? DART_RELEVANCE_LOW : _computeOldMemberRelevance(variable);
      relevance = request.opType.returnValueSuggestionsFilter(type, relevance);
      if (relevance == null) {
        return;
      }
    ...
    _add(_createSuggestion(variable, kind: kind, prefix: prefix, relevance: relevance));
  }
  ...
  void suggestTypeParameter(TypeParameterElement parameter) {
    ...
    if (request.useNewRelevance) {
      var elementKind = _computeElementKind(parameter);
      var isConstant = request.inConstantContext ? request.featureComputer.isConstantFeature(parameter) : -1.0;
      relevance = toRelevance(weightedAverage([elementKind, isConstant], [1.0, 1.0]), Relevance.typeParameter);
      listener?.computedFeatures(elementKind: elementKind);
    } else {
      relevance = _computeOldMemberRelevance(parameter);
    }

    _add(_createSuggestion(parameter, kind: CompletionSuggestionKind.IDENTIFIER, relevance: relevance));
  }
  ...
  void suggestUri(String uri) {
    int relevance;
    if (request.useNewRelevance) {
      relevance = uri == 'dart:core' ? Relevance.importDartCore : Relevance.import;
    } else {
      relevance = uri == 'dart:core' ? DART_RELEVANCE_LOW : DART_RELEVANCE_DEFAULT;
    }
    _add(CompletionSuggestion(CompletionSuggestionKind.IMPORT, relevance, uri, uri.length, 0, false, false));
  }
  ...
  double _computeElementKind(Element element) {
    ...
    var elementKind = request.featureComputer.elementKindFeature(element, location);
    if (elementKind < 0.0) {
      if (location == null) {
        listener?.missingCompletionLocationAt(request.target.containingNode, request.target.entity);
      } else {
  ...
  int _computeOldMemberRelevance(Element member) {
    ...
    if (!member.name.startsWith('_') && member.library == request.libraryElement) {
  ...
  int _computeTopLevelRelevance(Element element, {int defaultRelevance = 800, @required DartType elementType}) {
    ...
    //  signal.
    var featureComputer = request.featureComputer;
    var contextType = featureComputer.contextTypeFeature(request.contextType, elementType);
    var elementKind = _computeElementKind(element);
    var hasDeprecated = featureComputer.hasDeprecatedFeature(element);
    var isConstant = request.inConstantContext ? featureComputer.isConstantFeature(element) : -1.0;
    var relevance = toRelevance(
        weightedAverage([contextType, elementKind, hasDeprecated, isConstant], [1.0, 0.75, 0.2, 1.0]),
        defaultRelevance);
    listener?.computedFeatures(contextType: contextType, elementKind: elementKind, hasDeprecated: hasDeprecated);
    return relevance;
  }
  ...
  CompletionSuggestion _createSuggestion(Element element,
    ...
    var suggestion =
        CompletionSuggestion(kind, relevance, completion, completion.length, 0, element.hasOrInheritsDeprecated, false);
    ...
    if (element is ExecutableElement && element is! PropertyAccessorElement) {
      suggestion.parameterNames = element.parameters.map((ParameterElement parameter) => parameter.name).toList();
      suggestion.parameterTypes = element.parameters.map((ParameterElement parameter) {
        var paramType = parameter.type;
        // Gracefully degrade if type not resolved yet
        return paramType != null ? paramType.getDisplayString(withNullability: false) : 'var';
      }).toList();

      var requiredParameters = element.parameters.where((ParameterElement param) => param.isRequiredPositional);
      suggestion.requiredParameterCount = requiredParameters.length;

      var namedParameters = element.parameters.where((ParameterElement param) => param.isNamed);
      suggestion.hasNamedParameters = namedParameters.isNotEmpty;

      addDefaultArgDetails(suggestion, element, requiredParameters, namedParameters);
    }
    return suggestion;
  }
  ...
  InterfaceType _instantiateClassElement(ClassElement element) {
    ...
    var nullabilitySuffix =
        request.featureSet.isEnabled(Feature.non_nullable) ? NullabilitySuffix.none : NullabilitySuffix.star;
  ...
  void _setDocumentation(CompletionSuggestion suggestion, Element element) {
    var doc = DartUnitHoverComputer.computeDocumentation(request.dartdocDirectiveInfo, element);
    if (doc != null) {
...
abstract class SuggestionListener {
  ...
  void missingCompletionLocationAt(AstNode containingNode, SyntacticEntity entity);
