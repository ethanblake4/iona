...
class TypeMemberContributor extends DartCompletionContributor {
  ...
  Future<void> computeSuggestions(DartCompletionRequest request, SuggestionBuilder builder) async {
    ...
    if (expression == null || expression.isSynthetic || expression is ExtensionOverride) {
      return;
    }
    ...
    if (type == null || type.isDynamic) {
      ...
      if (expression is Identifier) {
        ...
        if ((type == null || type.isDynamic) && expression is SimpleIdentifier) {
    ...
    if (type is InterfaceType) {
      ...
      memberBuilder.buildSuggestions(type, mixins: mixins, superclassConstraints: superclassConstraints);
    }
  }
}
...
class _SuggestionBuilder extends MemberSuggestionBuilder {
  ...
  _SuggestionBuilder(DartCompletionRequest request, SuggestionBuilder builder) : super(request, builder);
  ...
  void buildSuggestions(InterfaceType type, {List<InterfaceType> mixins, List<InterfaceType> superclassConstraints}) {
    ...
    for (var targetType in types) {
      ...
      if (request.useNewRelevance) {
        inheritanceDistance = request.featureComputer.inheritanceDistanceFeature(type.element, targetType.element);
      }
      for (var method in targetType.methods) {
        // Exclude static methods when completion on an instance.
        if (!method.isStatic) {
          addSuggestionForMethod(method: method, inheritanceDistance: inheritanceDistance);
        }
      }
      for (var accessor in targetType.accessors) {
        if (!accessor.isStatic) {
          addSuggestionForAccessor(accessor: accessor, inheritanceDistance: inheritanceDistance);
        }
      }
