...
import '../suggestion_builder.dart';
...
class ArgListContributor extends DartCompletionContributor {
  ...
  void _addDefaultParamSuggestions(Iterable<ParameterElement> parameters, [bool appendComma = false]) {
    var appendColon = !_isInNamedExpression();
    var namedArgs = _namedArgs();
    for (var parameter in parameters) {
      if (parameter.isNamed) {
        _addNamedParameterSuggestion(namedArgs, parameter, appendColon, appendComma);
      }
    }
  }

  void _addNamedParameterSuggestion(
      List<String> namedArgs, ParameterElement parameter, bool appendColon, bool appendComma) {
    var name = parameter.name;
    if (name != null && name.isNotEmpty && !namedArgs.contains(name)) {
      builder.suggestNamedArgument(parameter, appendColon: appendColon, appendComma: appendComma);
    }
  }
  ...
  void _addSuggestions(Iterable<ParameterElement> parameters) {
    ...
    var requiredParam = parameters.where((ParameterElement p) => p.isRequiredPositional);
  ...
  void _buildClosureSuggestions(NamedExpression argument) {
    ...
    if (type is FunctionType) {
      builder.suggestClosure(type, includeTrailingComma: argument.endToken.next?.type != TokenType.COMMA);
    }
  }
  ...
  bool _isAppendingToArgList() {
    if (argumentList != null) {
      ...
      if (argumentList.arguments.isNotEmpty && argumentList.arguments.last == entity) {
  ...
  bool _isFollowedByAComma() {
    ...
    var entity = request.target.entity;
    var token = entity is AstNode ? entity.endToken : entity is Token ? entity : null;
    return (token != containingNode?.endToken) && token?.next?.type == TokenType.COMMA && !token.next.isSynthetic;
  }

  bool _isInFlutterCreation() {
    var flutter = Flutter.of(request.result);
    var containingNode = request.target?.containingNode;
    var newExpr = containingNode != null ? flutter.identifyNewExpression(containingNode.parent) : null;
    ...
    if (argumentList != null) {
      ...
      if (entity is SimpleIdentifier) {
        ...
        if (argumentList.arguments.length == argIndex + 1 ||
            argumentList.arguments.getRange(argIndex + 1, argIndex + 2).first is NamedExpression) {
          return true;
        }
      }
    }
