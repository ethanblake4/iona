...
import 'package:analyzer_plugin/protocol/protocol_common.dart' show CompletionSuggestionKind;
import 'package:analyzer_plugin/src/utilities/completion/completion_target.dart';
import 'package:analyzer_plugin/src/utilities/completion/optype.dart';
import 'package:analyzer_plugin/src/utilities/visitors/local_declaration_visitor.dart' show LocalDeclarationVisitor;
...
import 'package:iona_flutter/plugin/dart/completion/utilities/strings.dart';
...
class LocalReferenceContributor extends DartCompletionContributor {
  ...
  Future<void> computeSuggestions(DartCompletionRequest request, SuggestionBuilder builder) async {
    ...
    var suggestLocalFields = node is ConstructorDeclaration && node.initializers.contains(request.target.entity);
    ...
    if (!opType.isPrefixed) {
      if (opType.includeReturnValueSuggestions ||
        ...
        localVisitor = _LocalVisitor(request, builder, visibilityTracker, suggestLocalFields: suggestLocalFields);
        try {
    ...
    if (request.includeIdentifiers) {
      ...
      if (member != null) {
        ...
        if (classOrMixin is ClassOrMixinDeclaration && classOrMixin.declaredElement != null) {
  ...
  void _addSuggestionsForType(InterfaceType type, DartCompletionRequest request, double inheritanceDistance,
      ...
      for (var accessor in type.accessors) {
        if (visibilityTracker._isVisible(accessor.declaration)) {
          if (accessor.isGetter) {
            if (opType.includeReturnValueSuggestions) {
              memberBuilder.addSuggestionForAccessor(accessor: accessor, inheritanceDistance: inheritanceDistance);
            }
          } else {
      ...
      if (visibilityTracker._isVisible(method.declaration)) {
        if (method.returnType == null) {
          memberBuilder.addSuggestionForMethod(
              method: method, inheritanceDistance: inheritanceDistance, kind: classMemberSuggestionKind);
  ...
  void _computeSuggestionsForClass(ClassElement classElement, DartCompletionRequest request) {
    var isFunctionalArgument = request.target.isFunctionalArgument();
    classMemberSuggestionKind =
        isFunctionalArgument ? CompletionSuggestionKind.IDENTIFIER : CompletionSuggestionKind.INVOCATION;
    for (var type in classElement.allSupertypes) {
      double inheritanceDistance;
      if (request.useNewRelevance) {
        inheritanceDistance = request.featureComputer.inheritanceDistanceFeature(classElement, type.element);
      }
      _addSuggestionsForType(type, request, inheritanceDistance, isFunctionalArgument: isFunctionalArgument);
    }
  }
...
class _LocalVisitor extends LocalDeclarationVisitor {
  ...
  _LocalVisitor(this.request, this.builder, this.visibilityTracker, {@required this.suggestLocalFields})
    ...
    if (!useNewRelevance) {
      ...
      var data = request.result != null ? request.result.content : request.sourceContents;
  ...
  CompletionSuggestionKind get _defaultKind =>
      targetIsFunctionalArgument ? CompletionSuggestionKind.IDENTIFIER : opType.suggestKind;
  ...
  void declaredEnum(EnumDeclaration declaration) {
    if (visibilityTracker._isVisible(declaration.declaredElement) && opType.includeTypeNameSuggestions) {
      builder.suggestClass(declaration.declaredElement, kind: _defaultKind);
  ...
  void declaredField(FieldDeclaration fieldDecl, VariableDeclaration varDecl) {
    ...
    if ((visibilityTracker._isVisible(field) &&
      ...
      var enclosingClass = request.target.containingNode.thisOrAncestorOfType<ClassDeclaration>();
      if (enclosingClass != null) {
        inheritanceDistance =
            request.featureComputer.inheritanceDistanceFeature(enclosingClass.declaredElement, field.enclosingElement);
      }
  ...
  void declaredFunction(FunctionDeclaration declaration) {
    if (visibilityTracker._isVisible(declaration.declaredElement) &&
        (opType.includeReturnValueSuggestions || opType.includeVoidReturnSuggestions)) {
      if (declaration.isSetter) {
        if (!opType.includeVoidReturnSuggestions) {
          return;
        }
      } else if (!declaration.isGetter) {
        if (!opType.includeVoidReturnSuggestions && _isVoid(declaration.returnType)) {
          return;
        }
      }
      ...
      if (declaredElement is FunctionElement) {
        ...
        builder.suggestTopLevelPropertyAccessor(declaredElement, kind: _defaultKind);
      }
    }
  }
  ...
  void declaredFunctionTypeAlias(FunctionTypeAlias declaration) {
    if (opType.includeTypeNameSuggestions) {
      builder.suggestFunctionTypeAlias(declaration.declaredElement, kind: _defaultKind);
    }
  }
  ...
  void declaredLocalVar(SimpleIdentifier id, TypeAnnotation typeName) {
    if (visibilityTracker._isVisible(id.staticElement) && opType.includeReturnValueSuggestions) {
    ...
    if (visibilityTracker._isVisible(element) &&
      ...
      if (enclosingClass != null) {
        inheritanceDistance = request.featureComputer
            .inheritanceDistanceFeature(enclosingClass.declaredElement, element.enclosingElement);
      }
      if (element is MethodElement) {
        builder.suggestMethod(element, inheritanceDistance: inheritanceDistance, kind: _defaultKind);
      } else if (element is PropertyAccessorElement) {
        builder.suggestAccessor(element, inheritanceDistance: inheritanceDistance);
      }
    }
  }
  ...
  void declaredParam(SimpleIdentifier id, TypeAnnotation typeName) {
    ...
    if (visibilityTracker._isVisible(element) && opType.includeReturnValueSuggestions) {
  ...
  @override
  void declaredTopLevelVar(VariableDeclarationList varList, VariableDeclaration varDecl) {
    var variableElement = varDecl.declaredElement;
    if (visibilityTracker._isVisible(variableElement) && opType.includeReturnValueSuggestions) {
  ...
  void declaredTypeParameter(TypeParameter node) {
    if (visibilityTracker._isVisible(node.declaredElement) && opType.includeTypeNameSuggestions) {
...
class _VisibilityTracker {
  ...
  bool _isVisible(Element element) => element != null && declaredNames.add(element.name);
}
