...
import 'package:analyzer/src/dart/element/generic_inferrer.dart' show GenericInferrer;
...
class ExtensionMemberContributor extends DartCompletionContributor {
  ...
  Future<void> computeSuggestions(DartCompletionRequest request, SuggestionBuilder builder) async {
    ...
    if (expression == null) {
      var classOrMixin = request.target.containingNode.thisOrAncestorOfType<ClassOrMixinDeclaration>();
      if (classOrMixin != null) {
        var type = classOrMixin.declaredElement.thisType;
        _addExtensionMembers(containingLibrary, type);
      } else {
        var extension = request.target.containingNode.thisOrAncestorOfType<ExtensionDeclaration>();
        if (extension != null) {
            ...
            for (var type in types) {
              ...
              if (request.useNewRelevance) {
                inheritanceDistance = memberBuilder.request.featureComputer
                    .inheritanceDistanceFeature(extendedType.element, type.element);
              }
    ...
    for (var extension in containingLibrary.scope.extensions) {
      ...
      if (extendedType != null && typeSystem.isSubtypeOf(type, extendedType)) {
        ...
        if (memberBuilder.request.useNewRelevance) {
          inheritanceDistance =
              memberBuilder.request.featureComputer.inheritanceDistanceFeature(type.element, extendedType.element);
        }
  ...
  void _addInstanceMembers(ExtensionElement extension, double inheritanceDistance) {
    for (var method in extension.methods) {
      if (!method.isStatic) {
        memberBuilder.addSuggestionForMethod(method: method, inheritanceDistance: inheritanceDistance);
      }
    }
