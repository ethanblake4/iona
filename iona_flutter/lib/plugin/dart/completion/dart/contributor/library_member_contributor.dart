...
class LibraryMemberContributor extends DartCompletionContributor {
  ...
  void _buildSuggestions(
      DartCompletionRequest request, SuggestionBuilder builder, PrefixElement elem, List<ImportElement> imports) {
              ...
              if (element is ClassElement) {
                for (var constructor in element.constructors) {
                  if (!constructor.isPrivate) {
                    builder.suggestConstructor(constructor, kind: CompletionSuggestionKind.INVOCATION);
                  }
                }
              }
            } else {
              if (element is ClassElement || element is ExtensionElement || element is FunctionTypeAliasElement) {
                builder.suggestElement(element, kind: CompletionSuggestionKind.INVOCATION);
              } else if (!typesOnly && (element is FunctionElement || element is PropertyAccessorElement)) {
                builder.suggestElement(element, kind: CompletionSuggestionKind.INVOCATION);
              }
            }
          }
