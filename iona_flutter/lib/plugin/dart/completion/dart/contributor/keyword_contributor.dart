...
class _KeywordVisitor extends GeneralizingAstVisitor<void> {
  ...
  int get defaultKeyword => request.useNewRelevance ? 800 : DART_RELEVANCE_KEYWORD;

  Token get droppedToken => request.target.droppedToken;

  int get highKeyword => request.useNewRelevance ? 1000 : DART_RELEVANCE_HIGH;

  int get lowKeyword => request.useNewRelevance ? 400 : DART_RELEVANCE_KEYWORD - 1;
  ...
  void visitArgumentList(ArgumentList node) {
    ...
    if (entity is SimpleIdentifier && node.arguments.contains(entity)) {
      ...
      if (index > 0) {
        ...
        if (endToken?.lexeme == ')' && endToken.next?.lexeme == ',' && endToken.next.isSynthetic) {
          _addSuggestion(Keyword.ASYNC);
          _addSuggestion2(ASYNC_STAR);
          _addSuggestion2(SYNC_STAR);
        }
  ...
  void visitAsExpression(AsExpression node) {
    if (identical(entity, node.asOperator) && node.expression is ParenthesizedExpression) {
  ...
  void visitBlock(Block node) {
    ...
    if (entity is ExpressionStatement) {
      ...
      if (expression is SimpleIdentifier) {
        ...
        if (previous != null && previous.type == TokenType.CLOSE_PAREN && next.type == TokenType.OPEN_CURLY_BRACKET) {
          _addSuggestion(Keyword.ASYNC);
          _addSuggestion2(ASYNC_STAR);
          _addSuggestion2(SYNC_STAR);
        }
  ...
  void visitCompilationUnit(CompilationUnit node) {
    ...
    if (previousMember is ClassDeclaration) {
      if (previousMember.leftBracket == null || previousMember.leftBracket.isSynthetic) {
    ...
    if (previousMember == null || previousMember is Directive) {
      if (previousMember == null && !node.directives.any((d) => d is LibraryDirective)) {
  ...
  void visitFieldDeclaration(FieldDeclaration node) {
    ...
    if (node.fields.lateKeyword == null && request.featureSet.isEnabled(Feature.non_nullable)) {
  ...
  void visitForEachParts(ForEachParts node) {
    if (entity == node.inKeyword) {
      ...
      if (previous != null && previous.type == TokenType.EQ) {
        _addSuggestions([Keyword.CONST, Keyword.FALSE, Keyword.NULL, Keyword.TRUE]);
      } else {
  ...
  void visitFormalParameterList(FormalParameterList node) {
    var constructorDeclaration = node.thisOrAncestorOfType<ConstructorDeclaration>();
  ...
  void visitFunctionExpression(FunctionExpression node) {
    if (entity == node.body) {
      ...
      if (body is EmptyFunctionBody && node.parent is FunctionDeclaration && node.parent.parent is CompilationUnit) {
        _addCompilationUnitKeywords();
      }
    }
  }
  ...
  void visitImportDirective(ImportDirective node) {
    ...
    if ((entity == node.semicolon && node.uri != null && node.uri.offset + 1 != request.offset) ||
  ...
  void visitTryStatement(TryStatement node) {
    var obj = entity;
    if (obj is CatchClause || (obj is KeywordToken && obj.value() == Keyword.FINALLY)) {
      _addSuggestion(Keyword.ON);
      _addSuggestion(Keyword.CATCH);
      return null;
    }
  ...
  void _addSuggestion2(String keyword, {int offset, int relevance}) {
    builder.suggestKeyword(keyword, offset: offset, relevance: relevance ?? defaultKeyword);
  }
  ...
  bool _isEntityAfterIfWithoutElse(AstNode node) {
    ...
    if (entity is Statement) {
      ...
      if (entityIndex > 0) {
        ...
        return prevStatement is IfStatement && prevStatement.elseStatement == null;
      }
    }
    if (entity is Token) {
  ...
  static bool _isPreviousTokenSynthetic(Object entity, TokenType type) {
    if (entity is AstNode) {
      ...
      return previousToken != null && previousToken.isSynthetic && previousToken.type == type;
    }
    return false;
  }
}
