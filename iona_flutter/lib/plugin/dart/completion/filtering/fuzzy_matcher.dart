...
class FuzzyMatcher {
  ...
  ///   [:./ ]    PUNCT
  ///   otherwise NONE
  static const String TYPES = '0000000000000000000000000000000010000000000000112222222222100000'
  ...
  FuzzyMatcher(this.pattern, {this.matchStyle = MatchStyle.TEXT}) {
    ...
    scoreScale = pattern.isNotEmpty ? 1.0 / (maxCharScore * pattern.length) : 0.0;
  }
  ...
  int computeScore(String candidate, String candidateLower) {
    ...
    for (var i = 1; i <= candidate.length; i++) {
      ...
      for (var j = 0; j <= pattern.length; j++) {
        ...
        if (j == 0 || !(candidateLower.codeUnitAt(i - 1) == patternLower.codeUnitAt(j - 1))) {
          // Not a match.
          continue;
        }
        ...
        if (candidateRoles[i - 1] == CharRole.TAIL && patternRoles[j - 1] == CharRole.HEAD) {
          if (j > 1) {
          ...
          if (patternShort !=
              candidateLower.substring(i - 1, math.min(candidateLower.length, i - 1 + patternShort.length))) {
            continue;
          }
          charScore -= 4;
        }
        ...
        if ((candidate.codeUnitAt(i - 1) == pattern.codeUnitAt(j - 1)) ||
            (isHead && (!caseSensitive || patternRoles[j - 1] == CharRole.HEAD))) {
        ...
        for (var k = 0; k < 2; k++) {
          ...
          var isConsecutive = prevMatches || i - 1 == 0 || i - 1 == lastSegmentStart;
          ...
          if (!prevMatches && (candidateRoles[i - 1] == CharRole.TAIL || candidateRoles[i - 1] == CharRole.UC_TAIL)) {
    ...
    return scoreAt(candidate.length, pattern.length, bestLayerIndexAt(candidate.length, pattern.length));
  }
  ...
  void fuzzyMap(String string, List<CharRole> roles) {
    ...
    for (var i = 0; i < string.length; i++) {
      ...
      var type = ch < 128 ? _CharType.values[TYPES.codeUnitAt(ch) - 48] : _CharType.LOWER;
      var role = CharRole.NONE;
      if (type == _CharType.LOWER) {
        role = (prev.index <= _CharType.PUNCT.index) ? CharRole.HEAD : CharRole.TAIL;
      } else if (type == _CharType.UPPER) {
        role = CharRole.HEAD;
        // Note: this treats RPCTest as two words.
        if (prev == _CharType.UPPER &&
            !(i + 1 < string.length && string.codeUnitAt(i + 1) >= $a && string.codeUnitAt(i + 1) <= $z)) {
          role = CharRole.UC_TAIL;
        }
      } else if (type == _CharType.PUNCT) {
        if (matchStyle == MatchStyle.FILENAME && string[i] == '/' ||
            matchStyle == MatchStyle.SYMBOL && (string[i] == '.' || string[i] == ':' || string[i] == ' ')) {
    ...
    for (var i = string.length - 1; i >= 0 && roles[i] == CharRole.SEPARATOR; i--) {
  ...
  bool match(String candidate, String candidateLower) {
    ...
    if (matchStyle != MatchStyle.TEXT) {
      ...
      if (sep >= i) {
        ...
        return candidateLower.contains(patternLower.substring(patternLower.length - 1), sep);
      }
    }
    return true;
  }
