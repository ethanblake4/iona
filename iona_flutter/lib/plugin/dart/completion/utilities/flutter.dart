...
class Flutter {
  ...
  static final _uriFlutterMobileWidgets = Uri.parse('package:flutter/widgets.dart');

  static final _uriFlutterWebWidgets = Uri.parse('package:flutter_web/widgets.dart');
  ...
  void convertChildToChildren(
    ...
    if (!childArgSrc.contains(eol)) {
      ...
      String separator = getText(namedExp.offset, childArg.offset - namedExp.offset);
  ...
  void convertChildToChildren2(DartFileEditBuilder builder, Expression childArg, NamedExpression namedExp, String eol,
      Function getNodeText, Function getLinePrefix, Function getIndent, Function getText, Function rangeNode) {
    var childLoc = namedExp.offset + 'child'.length;
  ...
  NamedExpression findChildArgument(InstanceCreationExpression newExpr) =>
      newExpr.argumentList.arguments.firstWhere(isChildArgument, orElse: () => null);
  ...
  NamedExpression findChildrenArgument(InstanceCreationExpression newExpr) =>
      newExpr.argumentList.arguments.firstWhere(isChildrenArgument, orElse: () => null);
  ...
  InstanceCreationExpression identifyNewExpression(AstNode node) {
    ...
    if (node is SimpleIdentifier) {
      if (node.parent is ConstructorName && node.parent.parent is InstanceCreationExpression) {
        newExpr = node.parent.parent;
      } else if (node.parent?.parent is ConstructorName && node.parent.parent?.parent is InstanceCreationExpression) {
