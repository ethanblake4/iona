...
class CommonUsageSorter implements DartContributionSorter {
  ...
  void _update(CompletionRequest request, Iterable<CompletionSuggestion> suggestions) {
  ...
  void _updateInvocationRelevance(String typeName, LibraryElement libElem, Iterable<CompletionSuggestion> suggestions) {
    ...
    if (selectors != null) {
      for (var suggestion in suggestions) {
        ...
        if (element != null &&
            ...
            suggestion.kind == protocol.CompletionSuggestionKind.INVOCATION &&
...
class _BestTypeVisitor extends UnifyingAstVisitor<Element> {
  ...
  Element visitConstructorName(ConstructorName node) {
    return node.period != null && node.name == entity ? node.type?.type?.element : null;
  }
