...
class FlutterTool {
  ...
  FlutterTool._(this.startMode, this._sink, this._err, this._stream, this._kill, this._processCompleter) {
    init();
  }
  ...
  static FutureOr<FlutterTool> getInstance(String workingDirectory,
      {String path = '/Users/ethanelshyeb/fluttersdk/flutter/bin/flutter'}) async {
    if (_instance.containsKey(workingDirectory)) return _instance[workingDirectory];
  ...
  static Future<FlutterTool> create(
      {String path = '/Users/ethanelshyeb/fluttersdk/flutter/bin/flutter',
    ...
    final startArgs = startMode == FlutterToolStartMode.daemon
        ? ['daemon']
        : [startMode == FlutterToolStartMode.run ? 'run' : 'attach', '--machine'];
    final process =
        await Process.start(path, startArgs, workingDirectory: workingDirectory, mode: ProcessStartMode.normal);
  ...
  void init() {
    _streamSubscription = _stream.listen((messageStr) {
      ...
      try {
        final message = json.decode(messageStr.substring(1, messageStr.length - 1));
  ...
  void _handleEvent(Map eventMessage) {
    switch (eventMessage['event']) {
      case 'device.added':
        final device = standardDartSerializers.deserializeWith(FlutterDevice.serializer, eventMessage['params']);
        if (_onDeviceAddedListener != null) {
          _onDeviceAddedListener(device);
        }
        break;
      case 'daemon.connected':
        connectionInfo = standardDartSerializers.deserializeWith(DaemonInfo.serializer, eventMessage['params']);
  ...
  Future<Null> enableDevicePolling() => _internalSendMessage('device.enable').then((_) {});

  Future<String> _internalSendMessage(String method, [String serializedParams]) {
    final completer = Completer<String>();
    final id = _messageId++;
    _messageResponders[id] = completer;
    final paramsField = serializedParams == null ? '' : '"params":$serializedParams,';
