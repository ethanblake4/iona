...
extension AstNodeExtensions on AstNode {
  ...
  bool get hasOverride {
    ...
    if (node is AnnotatedNode) {
      for (var annotation in node.metadata) {
        if (annotation.name.name == 'override' && annotation.arguments == null) {
          return true;
        }
      }
    }
  ...
  bool get inForLoop => thisOrAncestorMatching((p) => p is ForStatement) != null;
...
extension FunctionBodyExtensions on FunctionBody {
  bool get isEmpty => this is EmptyFunctionBody || (this is BlockFunctionBody && beginToken.isSynthetic);
}
