...
//  final Function _deepEq = (a, b) => true;
  ...
  int commonLengthForOps(Map ma, Map mb) {
    ...
    if (a.hasNA || a.hasSubtype) alen++;

    if (b.hasNA || b.hasSubtype) blen++;
    ...
    for (var i = 0; i < alen; i++) {
      var p = a.path[i];
      if (i >= blen || p != b.path[i]) return null;
    }
  ...
  transformComponent(List dest, Map mc, Map motherC, String type) {
    ...
    if (c.hasNA || c.hasSubtype) cplength++;

    if (otherC.hasNA || otherC.hasSubtype) otherCplength++;
      ...
      // transform based on otherC
        ...
        if (c.hasLM) {
          if (commonOperand) {
            ...
            if (otherC.path[common] <= c.lm) c.lm++;
