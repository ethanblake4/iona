...
class AttributeList extends UnmodifiableListView<OpAttribute> {
  ...
  bool operator ==(otherAtts) {
    ...
    return (this as AttributeList).length == (otherAtts as AttributeList).length &&
        this.every((otherAtts as AttributeList).contains);
  }
    ...
    for (var i = 0; i < otherAtts.length; i++) {
      ...
      if (!found) {
        if (!isComposition && otherOp.isRemove) {
          throw new Exception(
              'trying to remove non-existing attribute: ${otherOp.opcode}${otherOp.key} from ${list.fold('', (p, n) => p + n.opcode + n.key)}');
        }
        list.add(otherOp);
      }
    }
      ...
      for (var j = 0; !skip && j < otherAtts.length; j++) {
        ...
        if (thisOp == otherOp) {
          ...
          if (thisOp.value.compareTo(otherOp.value) < 0) {
            ...
            res..add(new OpAttribute.remove(otherOp.key, otherOp.value))..add(thisOp);
          }
          skip = true;
    ...
    for (var i = 0; i < formatAtts.length; i++) {
      ...
      for (var j = 0; !skip && j < length; j++) {
        ...
        if (formatOp.key == thisOp.key && formatOp.value == thisOp.value) {
          ...
          res..add(new OpAttribute.remove(thisOp.key, thisOp.value))..add(formatOp);
          skip = true;
        }
      }
  ...
  String pack([List optPool]) {
    ...
    new List<OpAttribute>.from(this, growable: false)
      ...
      ..map((op) => new _Pair(op, _addToPool(pool, op))).forEach((item) {
        ...
        s..write(item.op.opcode)..write(_util.toString36(item.n));
      });
    return s.toString();
  }
